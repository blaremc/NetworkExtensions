...
namespace Transit.Addon.RoadExtensions.Roads.Common
{
    public static partial class RoadHelper
    {
        ...
        public static NetInfo.Lane SetupMedianLane(this NetInfo.Lane lane, LanesConfiguration config, NetInfoVersion version)
        {
        ...
        public static void SetupAnyDirectionProps(NetInfo.Lane lane, NetInfo.Lane source )
        {
       
            if (lane.m_laneProps == null || source.m_laneProps == null)
            {
                return;
            }

            if (lane.m_laneProps.m_props == null || source.m_laneProps.m_props == null)
            {
                return;
            }
            var prop = lane.m_laneProps.m_props.First().ShallowClone() ;
            ...
            var sfwd = source.m_laneProps.m_props.FirstOrDefault(p => p.m_flagsRequired == NetLane.Flags.Forward);
            var sleft = source.m_laneProps.m_props.FirstOrDefault(p => p.m_flagsRequired == NetLane.Flags.Left);
            var sright = source.m_laneProps.m_props.FirstOrDefault(p => p.m_flagsRequired == NetLane.Flags.Right);
            var newProps = new FastList<NetLaneProps.Prop>();

            var isLeftAllow = left != null || sleft != null;
            var isRightAllow = right != null || sright != null;
            var isForwardAllow = fwd != null || sfwd != null;
            /*
            Debug.Log("---VVVVV---");
            ...
            {
                Debug.Log("--LeftAllow");
                DebugOutputPanel.AddMessage(PluginManager.MessageType.Message, "LeftAllow");
            }
            if (isRightAllow)
            {
                Debug.Log("--isRightAllow");
                DebugOutputPanel.AddMessage(PluginManager.MessageType.Message, "isRightAllow");
            }
            if (isForwardAllow)
            {
                Debug.Log("--isForwardAllow");
                DebugOutputPanel.AddMessage(PluginManager.MessageType.Message, "isForwardAllow");
            }
            Debug.Log("---UUUUUU---");
            DebugOutputPanel.AddMessage(PluginManager.MessageType.Message, "-----" );
            */
            ...
            if (isForwardAllow)
            {
                ...
                if (isLeftAllow)
                {
                    if (isRightAllow)
                    {
                        prop.m_flagsRequired = NetLane.Flags.LeftForwardRight;
                    ...
                    {
                        prop.m_flagsRequired = NetLane.Flags.LeftForward;
                        prop.m_flagsForbidden = NetLane.Flags.Right;
                        newProps.Add(prop);
                        name = @"RoadArrowLF";
                    }
                }else
                {
                    if (isRightAllow)
                    {
                        prop.m_flagsRequired = NetLane.Flags.ForwardRight;
                        prop.m_flagsForbidden = NetLane.Flags.Left;
                        newProps.Add(prop);
                        name = @"RoadArrowFR";
                    }
                    else
                    {
                        prop.m_flagsRequired = NetLane.Flags.Forward;
                        prop.m_flagsForbidden = NetLane.Flags.LeftRight;
                        newProps.Add(prop);
                        name = @"RoadArrowF";
                    }
                }
            }else
            {
                if (isLeftAllow)
                {
                    if (isRightAllow)
                    {
                        prop.m_flagsRequired = NetLane.Flags.LeftRight;
                        prop.m_flagsForbidden = NetLane.Flags.Forward;
                        newProps.Add(prop);
                        name = @"RoadArrowLR";
                    }
                    else
                    {
                        prop.m_flagsRequired = NetLane.Flags.Left;
                        prop.m_flagsForbidden = NetLane.Flags.ForwardRight;
                        newProps.Add(prop);
                        name = @"RoadArrowL";
                    }
                }
                else
                {
                    if (isRightAllow)
                    {
                        prop.m_flagsRequired = NetLane.Flags.Right;
                        prop.m_flagsForbidden = NetLane.Flags.LeftForward;
                        newProps.Add(prop);
                        name = @"RoadArrowR";
                    }
                    else
                    {
                        prop.m_flagsRequired = NetLane.Flags.None;
                        prop.m_flagsForbidden = NetLane.Flags.LeftForwardRight;
                        newProps.Add(prop);
                        name = null;
                    }
                }
            ...
            if (name != null)
            {
                var arrow = Prefabs.Find<PropInfo>(name, false);

                tempProps.AddProp(new NetLaneProps.Prop
                {
                    m_prop = arrow,
            ...
            /*
            for (var i = 0; i< source.m_laneProps.m_props.Length; i++)
            {
                newProps.Add(source.m_laneProps.m_props[i]);
            }
            for (var i = 0; i < lane.m_laneProps.m_props.Length; i++)
            {
                newProps.Add(lane.m_laneProps.m_props[i]);
            }

            

            var fr = prop.ShallowClone();
            fr.m_flagsRequired = NetLane.Flags.ForwardRight;
            fr.m_flagsForbidden = NetLane.Flags.Left;
            newProps.Add(fr);

            var flr = prop.ShallowClone();
            flr.m_flagsRequired = NetLane.Flags.LeftForwardRight;
            flr.m_flagsForbidden = NetLane.Flags.None;
            newProps.Add(flr);

            var lr = prop.ShallowClone();
            lr.m_flagsRequired = NetLane.Flags.LeftRight;
            lr.m_flagsForbidden = NetLane.Flags.Forward;
            newProps.Add(lr);*/
            lane.m_laneProps.m_props = newProps.ToArray();
          ...
          /*  for (var i = 0; i <  lane.m_laneProps.m_props.Length; i++)
            {
                lane.m_laneProps.m_props[i].m_flagsRequired = NetLane.Flags.LeftForwardRight;
                lane.m_laneProps.m_props[i].m_flagsForbidden = NetLane.Flags.None;
            }
            return;*/
/*
            var fwd = lane.m_laneProps.m_props.FirstOrDefault(p => p.m_flagsRequired == NetLane.Flags.Forward);
           ...
           // var newProps = new FastList<NetLaneProps.Prop>();
            ...
            var l = prop.ShallowClone();
            l.m_flagsRequired = NetLane.Flags.Left;
            l.m_flagsForbidden = NetLane.Flags.ForwardRight;
            newProps.Add(l);

            var r = prop.ShallowClone();
            r.m_flagsRequired = NetLane.Flags.Right;
            r.m_flagsForbidden = NetLane.Flags.LeftForward;
            newProps.Add(r);

            var f = prop.ShallowClone();
            f.m_flagsRequired = NetLane.Flags.Forward;
            f.m_flagsForbidden = NetLane.Flags.LeftRight;
            newProps.Add(f);
